...
# @raycast.title Toggle DropRight Terminal
# @raycast.mode silent
#
# Optional parameters:
# @raycast.icon 🗔
...
# ドロップライト専用 wezterm(dropright.lua で起動した別プロセス)を
# AeroSpace のワークスペース移動でトグルする。dropdown/dropup と同じ仕組みの別インスタンス。
# - 未起動: dropright.lua で起動(gui-startup が画面右から45%に配置、AeroSpace ルールで floating)
# - フォーカス中: 退避ワークスペースへ移動(=消える)
# - 同じワークスペースにいる/退避中: 今のワークスペースへ呼び出してフォーカス

AEROSPACE=/opt/homebrew/bin/aerospace
WEZTERM_APP="/Applications/WezTerm.app"
CONFIG="$HOME/.config/wezterm/dropright.lua"
STASH_WS="dropr" # 退避先ワークスペース名(他のドロップ端末と別にする)

WID=$("$AEROSPACE" list-windows --all --format '%{window-id}|%{window-title}' | awk -F'|' '$2 == "dropright-terminal" {print $1; exit}')
...
if [ -z "$WID" ]; then
  ...
  if [ "$title" = "dropright-terminal" ]; then
